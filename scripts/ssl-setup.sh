...
# ---------------------------------------------------------------------------
# Step 8b: Generate nginx alias server blocks (generic, app-independent)
# ---------------------------------------------------------------------------
# Apps with nginx can include this file to serve alias domains.
# Each alias proxies to the primary domain's HTTPS port on localhost.
if [[ ${#ALL_SSL_DOMAINS[@]} -gt 1 ]]; then
    log "Generating nginx alias config: /tmp/.ssl-alias-nginx.conf"
    : > /tmp/.ssl-alias-nginx.conf
    for _d in "${ALL_SSL_DOMAINS[@]:1}"; do
        _alias_cert="/etc/letsencrypt/live/${_d}/fullchain.pem"
        _alias_key="/etc/letsencrypt/live/${_d}/privkey.pem"
        if [[ ! -f "$_alias_cert" || ! -f "$_alias_key" ]]; then
            warn "Skipping alias ${_d}: cert not found"
            continue
        fi


        cat >> /tmp/.ssl-alias-nginx.conf << NGINXALIAS

    # Alias domain: ${_d} (auto-generated by ssl-manager)
    server {
        listen ${SSL_HTTPS_PORT} ssl;
        http2 on;
        server_name ${_d};
        ssl_certificate ${_alias_cert};
        ssl_certificate_key ${_alias_key};
        ...
        location / {
            ...
            proxy_set_header X-Original-Host ${_d};
...
NGINXALIAS
        log "  Alias nginx block: ${_d} → localhost:${APP_HTTP_PORT}"
    done
fi
